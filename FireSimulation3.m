%% as a demo of dynamic programming of the cost, with labels of doors and rooms, only 1 vehicle with info is used



classdef FireSimulation3 < handle
    methods
        function run(self)
            ...
            for i=1:numel(rooms)
                text((rooms(i).topLeft(1)+rooms(i).bottomRight(1))/2, (rooms(i).topLeft(2)+rooms(i).bottomRight(2))/2, strcat('room ', int2str(rooms(i).id)));
            end
            for i=1:numel(doors)
                text(doors(i).x, doors(i).y + 0.2, strcat('door ', int2str(doors(i).id)));
            end
